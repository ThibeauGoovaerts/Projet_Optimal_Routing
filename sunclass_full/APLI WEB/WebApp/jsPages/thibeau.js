...
async function getQuiz(nb){
    let response = await fetch('https://api.trivia.willfry.co.uk/questions?limit=' + nb.toString());
    return await response.json();
}

async function getLoginLogs(limit){
    let response = await fetch('http://ec2-3-68-93-242.eu-central-1.compute.amazonaws.com:3000/login_show_logs/' + limit.toString());
    return await response.json();
}
...
async function fillPlan(dateValue){
    ...
    for (let i = 0; i < allChalets.length; i++) {
        ...
        }else if (indexAcceuil != -1 && acceuil[indexAcceuil].done === 1){
            div.className = "actionAllDone";
        }else if (indexCaution != -1 && indexControle == -1){
            div.className = "actionAllHalfDone";
        }else{
            div.className = "actionAll";
        }
        ...
        div.addEventListener("click", async () => {
            ...
            findings.innerHTML = "";
            let string = "<h1>" + allChalets[i] + "</h1><br>"
            if (indexCaution != -1){
            ...
            string += "<h2>Ajouter Remarque par rapport au chalet: </h2><input type='text' style='height: 75px;width: 75%; margin-bottom: 25px; font-size: 25px;' id='remarqueReception'></input><br><button id='addRemarqueReception'>Confirmer</button>"
...
async function login_logs(){
    let string = "<h1>Login Logs</h1>" +
        '<select id="quantity">' +
        ...
        '</select>' +
        '<select id="items">' +
            '<option value="item-1">Tout</option>' +
            '<option value="item-2">Forme Réduit</option>' +
        '</select><br>' +
        '<button id="confirmButton">Confirmer</button>' +
        "<div id='extraWhiteboard'>" +
        "</div>"
    document.getElementById("whiteboard").innerHTML = string
    document.getElementById('confirmButton').addEventListener('click', async () => {
        document.getElementById("extraWhiteboard").innerHTML = ""
        let logs = await getLoginLogs(document.getElementById('quantity').value);
        string = '<table style="width:100%" id="tableCautions"><tr>' +
            '<th class="Nr">Nom</th>' +
            '<th class="NrBungalow">Heure</th>' +
            '<th class="NrBungalow">Date</th>' +
            '</tr>'
        if (document.getElementById('items').value != 'item-2'){
            for (let i=0;i<logs.length;i++){
                let time = logs[i].date_time.split("T")
                string += '<tr id="adminTaches">' +
                    '<th class="Nr">' + logs[i].name + '</th>' +
                    '<th class="Nr">' + time[1].split('.')[0] + '</th>' +
                    '<th class="NrBungalow">' + time[0] + '</th>' +
                    '</tr>'
            }
        }else{
            let lastOne = ""
            for (let i=0;i<logs.length;i++){
                if (logs[i].name != lastOne){
                    let time = logs[i].date_time.split("T")
                    string += '<tr id="adminTaches">' +
                        '<th class="Nr">' + logs[i].name + '</th>' +
                        '<th class="Nr">' + time[1].split('.')[0] + '</th>' +
    ...
    let logs = await getLoginLogs(150);
    string = '<table style="width:100%" id="tableCautions"><tr>' +
        '<th class="Nr">Nom</th>' +
        '<th class="NrBungalow">Heure</th>' +
        '<th class="NrBungalow">Date</th>' +
...
async function passetempsQuiz(){
    document.getElementById("whiteboard").innerHTML = '';
    let result = await getQuiz(10)
    for (let i=0;i<result.length;i++){
        console.log(result[i])
        let div = document.createElement('div')
        div.className = "quizQuestion"
        div.innerHTML = "<h1>Q" + (i+1).toString() + ": " + result[i].question + "</h1>"
        let cacheArray = [...result[i].incorrectAnswers]
        cacheArray.push(result[i].correctAnswer)
        cacheArray = shuffleArray(cacheArray)
        for (let j = 0; j < cacheArray.length;j++){
            let answer = document.createElement('h3')
            answer.innerText = cacheArray[j]
            answer.className = "answer" + i.toString()
            div.appendChild(answer)
            answer.addEventListener('click', () => {
                let all = document.getElementsByClassName("answer" + i.toString())
                for (let k=0;k<all.length;k++){
                    if (result[i].correctAnswer === all[k].innerText){
                        if (cacheArray[j] === all[k].innerText){
                            all[k].style = "color: green; pointer-events: none; border-left: 1px dotted green;"
                        }else{
                            all[k].style = "pointer-events: none;"
                        }
                    }
                    else if(all[k].innerText === cacheArray[j]){
                        all[k].style = "color: red; pointer-events: none; border-left: 1px dotted red;"
                    }else{
                        all[k].style = "pointer-events: none;"
                    }
                }
...
async function main() {
    if (sessionStorage.getItem('Password') == null || sessionStorage.getItem("Member") == null){
        ...
        document.getElementById('login_logs').addEventListener("click", async () => {
            await login_logs();
        });
    }
